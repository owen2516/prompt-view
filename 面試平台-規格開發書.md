...
create table interview_sets (
  ...
  ai_model text default 'gemini-2.0-flash',   -- 此面試使用的 AI 模型
...
### 6.4 AI 模型設定
- MVP 階段固定使用 Gemini 系列模型（免費額度，例如 `gemini-2.0-flash`）；`ai_settings.default_model` / `interview_sets.ai_model` 欄位保留，UI 下拉選單先做但選項單一
