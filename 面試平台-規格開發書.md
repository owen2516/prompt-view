...
-- 註：MVP 階段僅支援 Claude 系列模型，default_model 欄位先固定/單選；多模型（如 GPT-4o）選擇為 Phase 5+ 規劃，屆時再開放此欄位的可選值
...
create table interview_sets (
  ...
  ai_model text default 'claude-sonnet-4-6',   -- 此面試使用的 AI 模型
...
1. **題目列表 tab**（預設）
   ...
   - 「新增題目」：可手動輸入，或點右上角**「AI 產生題目」**按鈕 → 輸入職缺/技能主題、難度、程式語言 → 呼叫 Claude API 產生結構化題目（標題＋描述＋起始程式碼）→ 主管確認/編輯後加入題目列表
...
3. **設定 tab**
   ...
   - AI 設定：AI 模型下拉選單（MVP 階段固定為 Claude，選單先做但選項單一，Phase 5+ 才開放多模型）、AI 系統提示詞文字框（定義 AI 在面試過程中的角色與行為準則，預設帶入「AI 設定」頁的全域預設值，可針對此面試覆蓋）
   - 「儲存設定」按鈕
...
- MVP 階段模型固定為 Claude，欄位保留擴充空間以便日後開放多模型選擇
...
- System Prompt 範例方向：要求 Claude 回傳結構化 JSON（`title`, `description`（markdown）, `starter_code`, `difficulty`）
...
- 後端 API Route：`POST /api/interview/[token]/chat`（後端驗證 token 有效後才呼叫 Claude API 並寫入 `ai_messages`）
...
### 6.4 AI 模型設定
- MVP 階段固定使用 Claude 系列模型；`ai_settings.default_model` / `interview_sets.ai_model` 欄位保留，UI 下拉選單先做但選項單一
- 建議統一透過後端封裝的 AI client 呼叫（`/lib/claude`），依 `ai_model` 欄位切換模型參數，方便 Phase 5+ 擴充其他供應商（如需支援 GPT 系列，需另外整合 OpenAI SDK 並在 `/lib` 下新增對應封裝）
...
- AI API 呼叫需在後端進行（不可將 Anthropic API Key 暴露於前端）
...
/lib
  ...
  /claude (Anthropic SDK 封裝：產題 / 對話 / 評分 三種 prompt)
  /email (Resend 封裝)
/types
```
...
- 主管端詳情頁顯示 AI 對話紀錄
- AI 設定頁（全域預設 Claude 模型與 prompt）＋ 面試模板設定 tab 的覆蓋設定
...
5. **AI 模型選擇**：未來要開放多模型可選（例如 Claude / GPT-4o），但 MVP 先固定使用 Claude，欄位與 UI 保留擴充空間（Phase 5）
